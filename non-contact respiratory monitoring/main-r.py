...
if len(sys.argv) == 2:
    ...
    webcam = cv2.VideoCapture(0)#摄像头编号
realWidth = 320
realHeight = 240
videoWidth = 160
videoHeight = 120
...
respiration_data = "Respiration Rate"
try:
...
# 打开串口
ser = serial.Serial("COM3", 115200)  # 根据实际情况，修改串口号和波特率
# 循环处理视频帧
while True:
    # 读取视频帧
    ret, frame = webcam.read()
    ...
    # 在适当的位置发送呼吸频率到串口
    rt = sum(resp[40:90])/50  # 计算呼吸频率
    rt = round(rt, 2)  # 保留两位小数
    data = {
        "Respiration Rate": rt
    }
    print(data)
    json_str = json.dumps(data)  # 将数据转换为JSON字符串
    ser.write(json_str.encode())  # 发送JSON字符串到串口
